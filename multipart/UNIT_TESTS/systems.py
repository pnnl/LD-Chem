...
from TraceGases import TraceGasPopulation, get_equilibrium_reactions
...
def update_particle_population(
    ...
    for ii,(particle,num_conc) in enumerate(zip(ParcelState_0.particle_population.particles,ParcelState_0.particle_population.num_concs)):
        ...
        if processes.cocondensation: 
            ...
            V_h2o = new_particle.get_mass_h2o()/new_particle.get_rho_w()
            for gas,conc in zip(ParcelState_0.TraceGas_population.gases,ParcelState_0.TraceGas_population.concs):
                Heff = gas.get_Heff(T0) # mol/m^3*Pa
                eq_reactions = get_equilibrium_reactions(gas.name, mechanism_data_path=mechanism_data_path)
            #     aq_conc = 
            #     dydt_gas = molec_masses[name]*water_volumes*kmt*(((y[Ig[name]]*100**3)/Na) - (conc[Gaq[name]*Np:Gaq[name]*Np+Np]/(H*R*T))) # kg/s
                
                
                # print(T0, gas.name, gas.H0*(1000/101325), Heff)
    ...
    feedbacks = Feedbacks(dwc_dt=dwc_dt, dwc_dt_next=dwc_dt_next, dwc=dwc, dwi_dt=dwi_dt)
