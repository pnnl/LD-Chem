...
def splat_setup(Npart=1, optimization_points=10000, mass_thresholds=None,
    ...
    while (sum(checks)!=len(checks) and counter<maxcounter):
        ...
        model_Dps=np.logspace(np.log10(np.min(Dp_uppers)), 3, 1000)
        for ii in range(len(particle_species)):
            ...
            if particle_type=='BC':
                ...
                CDF_calc-=np.min(CDF_calc)
                CDF_calc/=np.max(CDF_calc)
                #plt.plot(model_Dps, CDF_calc, '-r')
                #plt.title(particle_type)
            ...
            particle_num_concs[idx[1]]=sampled_Ns*100**3 # m^-3
        ...
        plt.errorbar(Dp_uppers, measured_N, fmt='o', yerr=N_error, mfc='w', mec='k', ecolor='k')
        params=[]
        for mode in range(modes):
         params.append(fitting_params[mode][0])
         params.append(fitting_params[mode][1])
         params.append(fitting_params[mode][2])
        plt.plot(model_Dps, Nmodal_lognormal(model_Dps, *params), '-r', zorder=100)
    ...
    while r2<0.98:
        ...
        try:
            ...
            for i in range(modes):
                p0.append(np.power(10, 4-i))
                p0.append(4*np.power(10, i+1))
                p0.append(1.2)
