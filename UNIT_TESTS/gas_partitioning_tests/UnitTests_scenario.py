...
from TraceGases import TraceGasPopulation, equilibrate_gases
...
from scenario import make_TraceGas_population, make_AqReactions, TrajectorySettings, Scenario
...
def create_constant_parcel(
    ...
    if cocondensation:
        TraceGas_population = make_TraceGas_population(gas_names, gas_conc, aq_reactions=aq_reactions, specdata_path=specdata_path)
    else:
    ...
    if aerosol_population == None:
        ...
        if TraceGas_population:
            aerosol_population = equilibrate_gases(aerosol_population,TraceGas_population,T0,P0)

        
    t_data=np.linspace(0, t_end, int(t_end/dt + 1))
    w_data=np.repeat(updraft_velocity, int(t_end/dt + 1))
    
    trajectories_settings = [TrajectorySettings(
            x0=None,y0=None,z0=0.0,
            u0=None,v0=None,w0=updraft_velocity,
            S0=S0, T0=T0, P0=P0,
            w_data=w_data, t_data=t_data,
